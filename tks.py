...
layout = [[psGUI.Text('Keywords :: ')], [psGUI.InputText()], [psGUI.Submit()]]
window = psGUI.Window('TwitterKeywordSentiment', layout)
...
while True:
    ...
    #     psGUI.popup("Not enough tweets!", f"In the last week there has not been enough tweets to produce valuable sentiment analysis for following keywords: {', '.join(keywords)}. Please, try again with different keywords or a lesser amount of keywords, since all of the specified keywords must appear in the tweet for it to be considered for analysis.")
    #     exit()
    #     # raise ValueError(
    ...
    #     # )
        ...
        window = psGUI.Window('TwitterKeywordSentiment', [[psGUI.Text('Keywords :: ')], [psGUI.InputText()], [psGUI.Submit()]])
        event, values = window.read()
        window.close()
...
if date_for_query != datetime.date.today():
    fig.suptitle(
        f"Sentiment of Tweets containing keywords: {', '.join(keywords).replace('[', '').replace(']', '')}\ndated for period: {date_for_query.strftime('%d.%m.%y')} - {datetime.date.today().strftime('%d.%m.%y')}",
        weight="bold",
    )
else:
    fig.suptitle(
        f"Sentiment of Tweets containing keywords: {', '.join(keywords).replace('[', '').replace(']', '')}\npublished today ({date_for_query.strftime('%d.%m.%y')})",
        weight="bold",
    )
...
for p in ax.patches:
    ...
    ax.set_ylim(ax.get_ylim()[0], ylim + 0.5)

graph = f"TwitterKeywordSentiment_{'_'.join(keywords).replace('[', '').replace(']', '')}_{datetime.date.today().strftime('%d_%m_%y')}.png"
